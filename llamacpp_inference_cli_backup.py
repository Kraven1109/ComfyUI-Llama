...
class ComfyLLama:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "cli_timeout": ("INT", {"default": 120, "min": 5, "max": 3600, "step": 5}),
    ...
    def inference_llamacpp(
        ...
        # Combine text inputs: prefix always first, then text_input if provided
        final_prompt = prefix
        if text_input:
            if final_prompt:
        ...
        if use_multimodal:
            ...
            arg_candidates = {
                ...
                "--no-display-prompt", "--context-shift"
            }
        ...
        try:
            ...
            # Reduce log verbosity for multimodal to suppress technical details
            if use_multimodal and "--log-verbosity" in arg_candidates:
                cmd.extend(["--log-verbosity", "0"])

            # Ignore EOS for text-only to prevent early stopping
            if not use_multimodal and "--ignore-eos" in arg_candidates:
                cmd.append("--ignore-eos")

            # Enable context shift for longer generations
            if "--context-shift" in arg_candidates:
            ...
            # Suppress performance details
            if "--no-perf" in arg_candidates:
                cmd.append("--no-perf")
            ...
            if strip_prompt and isinstance(prefix, str) and prefix.strip():
                try:
                    plain_prompt = prefix.strip()
